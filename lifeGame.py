...
name_male_list = ['John', 'Elliot', 'Paul', 'Mario', 'Eduardo', 'Bob', 'Paul The Panda', 'Ro The Hut', 'Ramster', 'Bill', 'Dude', 'Fred', 'Thomas', 'Dimebig the guitar big dude', 'David',
                  'Thor', 'Le Dog', 'WhoCares', 'Qwerty', 'Ricky', 'King of Spades', 'Java The Big Monster of Verbosity', 'Racedriver', 'Not So Mad Max', 'Arthur', 'Zakk The Barbarian', 'Martian Spy']

name_female_list = ['Maria', 'Danielle', 'Nina', 'Kitty', 'Debbie', 'Marie', 'Kassandra', 'Ana', 'Paty', 'Kenga', 'Queen', 'Brianna', 'Mina', 'Ferggie from Korea', 'Mioko',
                    'Sabrinna', 'Vilma', 'NineVolt', 'Monica', 'Livia Burned', 'Fanny', 'Babe', 'Queen of Python', 'Ariel', 'Tatooed Girl', 'Pinup Queen', 'Chick', 'Queen of Spades',]

causes_death_list = ['infected mushroom', 'a lighting went straight for the head', 'poisoned apple', 'sadness', 'game over', 'bad joke', 'stage diving', 'moshing with a barbarian', 'jumped too many holes', 'followed the rabbit to the wrong hole', 'forgot to define the type on a Java variable',
                     'played Pokemoon while crossing the road', 'broke the 8th string on a djent riff', 'missed a Meshuggah concert in town', 'too much happiness at the same time', 'lost the heart in a gamble', 'went to the crossroad and signed the wrong contract', ]
...
def set_name():
    ...
    if gender == "female":
        name_index_female = int(random.randint(
            0, int(len(name_female_list))) - 1)
...
def natural_death():
    print("\n======== R.I.P ========")
    print("\"" + str(name) + "\"" +
          " just passed away by natural causes, had great fun and a long life.")
